...
export function App() {
  ...
  const handleOnboarding = async (next: UserParams): Promise<void> => {
    ...
    try {
      ...
      trackEvent('norm_calculated', { goal: next.goal });
    ...
    return (
      <div>
        {saveError && (
          <p
            role="alert"
            className="mx-auto mt-4 max-w-md rounded-2xl bg-warn/5 px-4 py-3 text-center text-sm font-medium text-warn"
          >
            {saveError}
          </p>
        )}
  ...
  return (
    <>
      ...
      <DiaryPage
        ...
        entries={entries}
        onAddEntry={(e) => {
          trackEvent('entry_added', { meal: e.meal });
          void addEntry(e);
        }}
