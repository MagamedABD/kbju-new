    ...
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-5">
      ...
      {!isSupabaseConfigured && (
        <div
          ...
          role="note"
        >
          Backend не настроен: задайте VITE_SUPABASE_URL и VITE_SUPABASE_ANON_KEY в переменных
          окружения.
        </div>
      )}
      ...
      {error && (
        <p
          role="alert"
          className="mt-3 rounded-2xl bg-warn/5 px-4 py-3 text-sm font-medium text-warn"
        >
          {error}
        </p>
      )}
      {notice && (
        <p className="mt-3 rounded-2xl bg-brand-soft px-4 py-3 text-sm font-medium text-brand">
          {notice}
        </p>
      )}
