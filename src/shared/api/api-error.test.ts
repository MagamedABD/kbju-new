...
describe('toUserMessage', () => {
  it('переводит неверные учётные данные', () => {
    expect(toUserMessage({ message: 'Invalid login credentials' })).toBe('Неверный email или пароль.');
  });

  it('распознаёт уже зарегистрированный email', () => {
    expect(toUserMessage({ message: 'User already registered' })).toBe('Этот email уже зарегистрирован.');
  });
