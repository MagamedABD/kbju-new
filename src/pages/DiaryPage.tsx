...
export function DiaryPage({ norm, entries, onAddEntry, onRemoveEntry, onEditGoal, onSignOut }: DiaryPageProps) {
    ...
    <div className="mx-auto w-full max-w-md px-4 pb-10 pt-8">
      <header className="mb-6">
        ...
        <h1 className="mt-1 text-[26px] font-bold tracking-tight" data-testid="remaining-heading">
          {over ? `Превышение на ${total.calories - norm.calories} ккал` : `Осталось ${remaining} ккал`}
        </h1>
        <div className="mt-1 flex items-center gap-3 text-sm text-muted">
          ...
          {onSignOut && (
            <button type="button" onClick={onSignOut} className="ml-auto underline-offset-2 hover:underline">
              Выйти
            </button>
          )}
        </div>
